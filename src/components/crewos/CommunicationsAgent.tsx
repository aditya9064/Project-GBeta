...
export function CommunicationsAgent() {
  ...
  const handleToggleThread = useCallback((msgId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (expandedThreadId === msgId) {
      setExpandedThreadId(null);
    } else {
      setExpandedThreadId(msgId);
      actions.fetchThread(msgId);
    }
  }, [expandedThreadId, actions]);
          ...
          {isLoading ? (
            ...
            <>
              ...
              {individualMessages.map(msg => (
                    ...
                    <div className="comms-list-item-content">
                      ...
                      <div className="comms-list-item-bottom">
                        ...
                        {msg.threadCount && msg.threadCount > 0 && (
                          <span
                            className="comms-list-thread-count"
                            style={{ cursor: 'pointer' }}
                            onClick={e => handleToggleThread(msg.id, e)}
                          >
                            {expandedThreadId === msg.id ? <ChevronDown size={12} /> : <MessageSquare size={12} />}
                            {msg.threadCount}
                          </span>
                        )}
                      </div>
                ...
                {expandedThreadId === msg.id && threadMessages.length > 0 && (
                  <div style={{ paddingLeft: 48, borderLeft: '2px solid rgba(255,255,255,0.06)', marginLeft: 24, marginBottom: 4 }}>
                    {threadMessages.map(reply => (
                      <div
                        key={reply.id}
                        className="comms-list-item"
                        style={{ paddingTop: 6, paddingBottom: 6, opacity: 0.85, borderBottom: '1px solid rgba(255,255,255,0.04)' }}
                        onClick={() => handleSelectMessage(reply)}
                      >
                        <div className="comms-list-item-left">
                          <div className="comms-list-avatar" style={{ background: reply.fromColor, width: 28, height: 28, fontSize: 11 }}>
                            {reply.fromInitial}
                          </div>
                          <div className="comms-list-item-content">
                            <div className="comms-list-item-top">
                              <span className="comms-list-from" style={{ fontSize: 12 }}>{reply.from}</span>
                              <span className="comms-list-time" style={{ fontSize: 11 }}>{reply.relativeTime}</span>
                            </div>
                            <div className="comms-list-preview" style={{ fontSize: 12 }}>{reply.preview}</div>
                          </div>
                        </div>
                      </div>
                    ))}
