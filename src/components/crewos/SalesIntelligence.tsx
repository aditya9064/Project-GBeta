...
export function SalesIntelligence() {
  ...
  // Live data from backend (falls back to hardcoded sample data)
  const [liveLeads, setLiveLeads] = useState<Lead[]>(sampleLeads);
  const [livePipeline, setLivePipeline] = useState<PipelineStage[]>(pipelineStages);
  const [liveForecasts, setLiveForecasts] = useState<Forecast[]>(forecasts);
  const [liveInsights, setLiveInsights] = useState<AIInsight[]>(aiInsights);
    ...
    async function fetchSalesData() {
      ...
      try {
        ...
        if (leadsRes?.ok) {
          ...
          if (data.success && data.leads?.length > 0) setLiveLeads(data.leads);
        }
        if (pipelineRes?.ok) {
          const data = await pipelineRes.json();
          if (data.success && data.stages?.length > 0) setLivePipeline(data.stages);
        }
        if (forecastsRes?.ok) {
          const data = await forecastsRes.json();
          if (data.success && data.forecasts?.length > 0) setLiveForecasts(data.forecasts);
        }
        if (insightsRes?.ok) {
  ...
  const handleSyncCRM = useCallback(async () => {
    ...
    try {
      ...
      setSalesError('Failed to sync CRM data');
    } finally {
      setSalesLoading(false);
    }
  }, []);

  // AI scoring via backend (falls back to simulated progress)
      ...
      try {
        const progressInterval = setInterval(() => {
          setScoringProgress(prev => {
            if (prev >= 90 || !scoringRef.current) return prev;
      ...
      <div className="sales-insights-section">
        <div className="sales-section-header">
          <div className="sales-section-header-left">
            <Sparkles size={18} />
            <h3>AI Insights</h3>
            <span className="sales-section-badge">{liveInsights.length} active</span>
          </div>
    ...
    <div className="sales-pipeline-view">
      ...
      <div className="sales-funnel-section">
        ...
        <div className="sales-funnel">
          {livePipeline.map((stage, i) => {
            const maxValue = Math.max(...livePipeline.map(s => s.value));
      ...
      <div className="sales-conversion-section">
        ...
        <div className="sales-conversion-flow">
          {livePipeline.slice(0, -1).map((stage, i) => {
            const next = livePipeline[i + 1];
            return (
    ...
    <div className="sales-page">
      ...
      <div className="sales-header">
        ...
        <div className="sales-header-right">
          ...
          <button className="sales-btn-secondary" onClick={handleSyncCRM} disabled={salesLoading}>
            <RefreshCw size={15} className={salesLoading ? 'sales-spin' : ''} />
            {salesLoading ? 'Syncing...' : 'Sync CRM'}
          </button>
        </div>
      </div>
