...
function App() {
  ...
  // Handle logout via URL hash
  useEffect(() => {
    if (location.hash === '#logout') {
      signOut();
      navigate('/', { replace: true });
    }
  }, [location.hash, navigate, signOut]);

  // Show loading state while checking auth
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: '#0a0a0f', color: '#e07a3a', fontSize: '18px' }}>
        Loading...
      </div>
    );
  }
  ...
  // Authenticated — render CrewOS Dashboard
