...
export function useCommsAgent(): [CommsAgentState, CommsAgentActions] {
  ...
  const prevConnectionsRef = useRef<ChannelConnection[]>([]);
  ...
  // Auto voice learning: trigger analyzeStyle when a new channel connects
  useEffect(() => {
    const prev = prevConnectionsRef.current;
    const newlyConnected = connections.some(conn =>
      conn.status === 'connected' &&
      !prev.some(p => p.channel === conn.channel && p.status === 'connected')
    );
    prevConnectionsRef.current = connections;
    if (newlyConnected && messages.length > 0 && !isAnalyzingStyle && !styleAnalyzed) {
      analyzeStyle();
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [connections, messages.length, isAnalyzingStyle, styleAnalyzed]);
  ...
  const loadFromBackend = useCallback(async () => {
    ...
    try {
      ...
      if (msgData && msgData.messages.length > 0) {
        // Browser notifications for new VIP messages
        if (typeof Notification !== 'undefined' && Notification.permission === 'granted' && prevMessageIdsRef.current.size > 0) {
          for (const msg of msgData.messages) {
            if (!prevMessageIdsRef.current.has(msg.id)) {
              const isVipSender = vipContacts.some(
                v => v.name === msg.from || (v.email && msg.fromEmail && v.email.toLowerCase() === msg.fromEmail.toLowerCase())
              );
              if (isVipSender) {
                new Notification('New message from ' + msg.from, { body: msg.subject || msg.preview });
              }
            }
          }
        }
        prevMessageIdsRef.current = new Set(msgData.messages.map(m => m.id));
