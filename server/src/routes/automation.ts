...
/* ═══════════════════════════════════════════════════════════
   Agent Management Routes — CRUD + execution logs
   
   Persists agents to Firestore (collection: agents/{agentId})
   with an in-memory Map fallback when Firestore is unavailable.
   
   GET    /api/agents            — List agents (optionally by userId)
   POST   /api/agents            — Create or update an agent
   PUT    /api/agents/:id        — Update agent status/config
   DELETE /api/agents/:id        — Delete an agent
   GET    /api/agents/:id/logs   — Get execution logs for an agent
   POST   /api/agents/:id/logs   — Save an execution log
   ═══════════════════════════════════════════════════════════ */
...
/* ─── Firestore setup (reuses existing app if already initialised) ─── */

let agentsDb: Firestore | null = null;
let agentsFirestoreOk = false;
...
async function checkAgentsFirestore(): Promise<boolean> {
  if (!agentsDb || !agentsFirestoreOk) return false;
  try {
    await agentsDb.collection(AGENTS_COL).doc('_health').get();
    return true;
  } catch {
    agentsFirestoreOk = false;
    return false;
  }
}
...
agentsRouter.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (await checkAgentsFirestore()) {
      let query: FirebaseFirestore.Query = agentsDb!.collection(AGENTS_COL);
    ...
    let agents = Array.from(memAgents.values());
...
agentsRouter.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (await checkAgentsFirestore()) {
      const logsSnap = await agentsDb!.collection(AGENTS_COL).doc(id).collection('logs').get();
      const batch = agentsDb!.batch();
      logsSnap.docs.forEach((d) => batch.delete(d.ref));
      batch.delete(agentsDb!.collection(AGENTS_COL).doc(id));
      await batch.commit();
    }
...
agentsRouter.post('/:id/logs', async (req: Request, res: Response) => {
  try {
    ...
    const entry = { ...log, id: logId };

    if (!memLogs.has(id)) memLogs.set(id, []);
    const agentLogs = memLogs.get(id)!;
    agentLogs.unshift(entry);
    if (agentLogs.length > 50) agentLogs.length = 50;

    if (await checkAgentsFirestore()) {
      await agentsDb!.collection(AGENTS_COL).doc(id).collection('logs').doc(logId).set(entry);
    }
